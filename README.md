[![Build Status](https://falcon.ecg.rice.edu:443/buildStatus/icon?job=github_public_agora%2Fmulti-cell)](https://falcon.ecg.rice.edu:443/job/github_public_agora/job/multi-cell/)

# NEXUS
Please check out our paper [NEXUS](https://arxiv.org/abs/2509.04625). 

NEXUS is a real-time multi-cell baseband processing framework built on top of [Savannah](#Savannah).
...
* Configure ACC100 properly for multile virtual functions ([VFs tutorial](VFs_README.md)).
...
CMake options select the thread models, arithmetic libraries, LDPC decoder, etc.
...
| `MAT_OP_TYPE`    | ARMA_CUBE <br> ARMA_VEC <br> AVX512 |
...
The configuration is a JSON script that is related.
...
We recommend using the COSMOS Testbed for the IBM 28 GHz phased array antenna module (PAAM).
For a local testbed, we recommend using the USRP X310 SDR to set up FR2 parameters with sub-7 GHz antennas for radio frequency.
...
The configuration is json script that is related.
We can set up MIMO dimension (`base_radio_num`/`ue_radio_num`), FFT size (`fft_size`), number of data subcarriers (`ofdm_data_num`), modulation scheme (`modulation`), LDPC code rate (`code_rate`), and sampling rate (`sample_rate`).
