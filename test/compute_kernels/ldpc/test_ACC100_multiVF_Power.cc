...
struct rte_bbdev_op_data** inputs_2;
struct rte_bbdev_op_data** hard_outputs_2;
...
int main(int argc, char* argv[]) {
  ...
  const char* rte_argv[] = {
      "txrx", "-l",      core_list.c_str(), "-a",           "18:00.1",
      "-a",   "18:00.2", "--log-level",     "lib.eal:info", nullptr};
  ...
  bbdev_op_pool_2 =
      rte_bbdev_op_pool_create("bbdev_op_pool_dec_2", RTE_BBDEV_OP_LDPC_DEC,
                               NB_MBUF, CACHE_SIZE, rte_socket_id());
  ...
  ret = rte_bbdev_setup_queues(1, NUM_QUEUES, info.socket_id);
  if (ret < 0) {
    printf("rte_bbdev_setup_queues(%u, %u, %d) ret %i\n", 1, NUM_QUEUES,
           rte_socket_id(), ret);
  }
  ...
  for (int q_id = 0; q_id < NUM_QUEUES; q_id++) {
    /* Configure all queues belonging to this bbdev device */
    ret = rte_bbdev_queue_configure(1, q_id, &qconf);
    if (ret < 0)
      rte_exit(EXIT_FAILURE,
               "ERROR(%d): BBDEV %u queue %u not configured properly\n", ret, 1,
               q_id);
  ...
  in_mbuf_pool_2 = rte_pktmbuf_pool_create(
      "in_pool_0_2", NUM_ELEMENTS_IN_MEMPOOL, 0, 0, DATA_ROOM_SIZE, 0);
  out_mbuf_pool_2 = rte_pktmbuf_pool_create(
      "hard_out_pool_0_2", NUM_ELEMENTS_IN_MEMPOOL, 0, 0, DATA_ROOM_SIZE, 0);
  ...
  ops_mp =
      rte_bbdev_op_pool_create("RTE_BBDEV_OP_LDPC_DEC_1", RTE_BBDEV_OP_LDPC_DEC,
  ...
  ops_mp_2 =
      rte_bbdev_op_pool_create("RTE_BBDEV_OP_LDPC_DEC_2", RTE_BBDEV_OP_LDPC_DEC,
                               NUM_ELEMENTS_IN_POOL, OPS_CACHE_SIZE, socket_id);
  if (ops_mp_2 == nullptr) {
    std::cerr << "Error: Failed to create memory pool for bbdev operations."
              << std::endl;
  ...
  ret_socket_in = test_allocate_buffers_on_socket(
      inputs_2, 1 * sizeof(struct rte_bbdev_op_data), 0);
  ret_socket_hard_out = test_allocate_buffers_on_socket(
      hard_outputs_2, 1 * sizeof(struct rte_bbdev_op_data), 0);
  if (ret_socket_in != TEST_SUCCESS || ret_socket_hard_out != TEST_SUCCESS) {
    rte_exit(EXIT_FAILURE, "Failed to allocate socket\n");
  }
  ...
  for (int i = 0; i < iter_num; i++) {
    ...
    ref_dec_op_2[i]->ldpc_dec.basegraph = config.basegraph;
    ref_dec_op_2[i]->ldpc_dec.z_c = config.z_c;
    ref_dec_op_2[i]->ldpc_dec.n_filler = config.n_filler;
    ref_dec_op_2[i]->ldpc_dec.rv_index = config.rv_index;
    ref_dec_op_2[i]->ldpc_dec.n_cb = config.n_cb;
    ref_dec_op_2[i]->ldpc_dec.q_m = config.q_m;
    ref_dec_op_2[i]->ldpc_dec.code_block_mode = config.code_block_mode;
    ref_dec_op_2[i]->ldpc_dec.cb_params.e = config.e;
    ref_dec_op_2[i]->ldpc_dec.iter_max = config.iter_max;
    ref_dec_op_2[i]->opaque_data = (void*)(uintptr_t)i;
  }
  ...
  // char filename[150];
  // get_timestamped_filename(filename, sizeof(filename), folder, config_1, config_2, config_3, config_4, iter_num);

  struct rte_bbdev_op_ldpc_dec* ops_td;
  struct rte_bbdev_op_data* hard_output;
  struct rte_mbuf* temp_m;
  size_t memcmp_ret = 0;
  int test_iter_num = 10000000;
  ...
  for (int j = 0; j < test_iter_num; j++) {
    ...
    char* data_2;
    struct rte_bbdev_op_data* bufs_2 = *inputs_2;
    rte_bbdev_op_data* bufs_out_2 = *hard_outputs_2;

    struct rte_mbuf* m_head;
    struct rte_mbuf* m_head_out;

    struct rte_mbuf* m_head_2;
    struct rte_mbuf* m_head_out_2;
    ...
    for (int i = 0; i < iter_num; i++) {
      ...
      // for 2nd VF
      m_head_2 = rte_pktmbuf_alloc(in_mbuf_pool_2);

      bufs_2[0].data = m_head_2;
      bufs_2[0].offset = 0;
      bufs_2[0].length = 0;
      data_2 = rte_pktmbuf_append(m_head_2, config.n_cb);

      // Copy data from demod_data to the mbuf
      rte_memcpy(data_2, input_values + (0 * config.n_cb), config.n_cb);
      bufs_2[0].length += config.n_cb;

      m_head_out_2 = rte_pktmbuf_alloc(out_mbuf_pool_2);

      bufs_out_2[0].data = m_head_out_2;
      bufs_out_2[0].offset = 0;
      bufs_out_2[0].length = 0;

      bufs_out_2[0].length += config.n_cb;

      // parse data
      ref_dec_op[i]->ldpc_dec.input = *inputs[0];
      ref_dec_op[i]->ldpc_dec.hard_output = *hard_outputs[0];

      ref_dec_op_2[i]->ldpc_dec.input = *inputs_2[0];
      ref_dec_op_2[i]->ldpc_dec.hard_output = *hard_outputs_2[0];

      rte_pktmbuf_free(m_head);
      rte_pktmbuf_free(m_head_out);

      rte_pktmbuf_free(m_head_2);
      rte_pktmbuf_free(m_head_out_2);

      enq += rte_bbdev_enqueue_ldpc_dec_ops(0, 0, &ref_dec_op[i], 1);
      enq_2 += rte_bbdev_enqueue_ldpc_dec_ops(1, 0, &ref_dec_op_2[i], 1);
    }
    ...
    int retry_count = 0;
    while (deq < enq && deq_2 < enq_2 && retry_count < 100000000) {
      deq += rte_bbdev_dequeue_ldpc_dec_ops(0, 0, &ops_deq[deq], enq - deq);
      deq_2 += rte_bbdev_dequeue_ldpc_dec_ops(1, 0, &ops_deq_2[deq_2],
                                              enq_2 - deq_2);
      retry_count++;
    }
  ...
  // for (int j = 0; j < test_iter_num; j++) {
  //     fprintf(file, "%d,%f,%d,%ld\n", j, results[j].time, results[j].retry_count, results[j].BLER);
  // }
