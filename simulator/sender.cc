...
Sender::Sender(Config* cfg, size_t socket_thread_num, size_t core_offset,
  ...
  mbuf_pool_ =
      DpdkTransport::CreateMempool(cfg->DpdkNumPorts(), cfg->PacketLength(), cfg->bbdev_id_1());
...
void Sender::StartTx() {
  ...
  std::cout << "socket_thread_num_ is " << socket_thread_num_ << std::endl;
      ...
      if (packet_count_per_symbol_[comp_frame_slot][ctag.symbol_id_] ==
        ...
        if (next_symbol_id == cfg_->Frame().NumTotalSyms()) {
          ...
          if ((kDebugSenderReceiver == true) ||
            ...
            AGORA_LOG_INFO(
                "Cell # %d:\tSender: Tx frame %d in %.2f ms, next frame %zu, "
                "start symbol "
                "%zu\n",
                (int)cfg_->bbdev_id_1(), ctag.frame_id_,
                (frame_end_us - frame_start_us) / 1000.0, next_frame_id,
                next_symbol_id);
          }
...
void* Sender::WorkerThread(int tid) {
  ...
  for (size_t radio_number = radio_lo; radio_number <= radio_hi;
       radio_number++) {
    std::cout << "bsrru port is: " << cfg_->BsRruPort() + radio_number
              << std::endl;
