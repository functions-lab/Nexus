...
void DpdkTransport::DpdkInit(uint16_t core_offset, size_t thread_num) {
  ...
  //     std::to_string(55);  // this is hard set to core 36
  // for (size_t i = 1; i < thread_num + 1; i++) {
  //   core_list =
  //       core_list + "," + std::to_string(GetPhysicalCoreId(core_offset + i));
  // }

  // std::cout <<"core lis is: " << core_list << std::endl;

  // const char* rte_argv[] = {
  //     "txrx", "-l", core_list.c_str(), "-a", "18:00.0", "-a", "18:00.1", "-a",
  //     "18:00.2", "-a", "18:00.3", "-a", "18:00.4", "-a", "18:00.5", "-a",
  //     "18:00.6", "-a", "18:00.7",
  //     // "-a", "99:00.0",
...
rte_mempool* DpdkTransport::CreateMempool(size_t num_ports,
                                          size_t packet_length, uint8_t bbdev_id) {
  const size_t mbuf_size = packet_length + kPayloadOffset + kMBufCacheSize;
  std::string pool_name = "MBUF_POOL" + std::to_string(bbdev_id);
  rte_mempool* mbuf_pool =
      rte_pktmbuf_pool_create(pool_name.c_str(), kNumMBufs * num_ports,
