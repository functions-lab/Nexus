...
static inline void ReverseAdaptBitsForMod(uint8_t* bytes_in,
                                          uint8_t* bit_seq_out, size_t len,
                                          size_t mod_type) {
  uint32_t bits =
      0;  // Use a 32-bit accumulator for more efficient bit handling
  size_t bits_avail = 0;               // Number of valid bits in [bits]
  ...
  if (bits_avail > 0) {
    *bit_seq_out++ = Bitreverse8(
        static_cast<uint8_t>(bits >> 24));  // Reverse and store remaining bits
  }
}
