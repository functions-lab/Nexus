...
Config::Config(std::string jsonfilename)
  ...
  acc100_addr_1_ = tdd_conf.value("acc100_addr_1", "18:00.0");
  bbdev_id_1_ = tdd_conf.value("bbdev_id_1", 0);
  solution_ = tdd_conf.value("solution", "sc");
  ...
  if (small_mimo_acc_) {
    RtAssert(
        (bs_ant_num_ == 1 && ue_ant_num_ == 1) ||
            (bs_ant_num_ == 2 && ue_ant_num_ == 2) ||
            (bs_ant_num_ == 4 && ue_ant_num_ == 4) ||
            (bs_ant_num_ == 8 && ue_ant_num_ == 8),
        "Small MIMO Acceleration is only supported for 1x1/2x2/4x4/8x8 MIMO");
  }
...
void Config::GenData() {
  ...
  const std::string ul_encoded_data_file =
      ...
      std::to_string(this->ue_ant_total_) + ".bin";
  std::ifstream infile(ul_encoded_data_file,
    ...
    for (size_t j = 0; j < ue_ant_num_; j++) {
      for (size_t k = 0; k < ul_ldpc_config_.NumBlocksInSymbol(); k++) {
        ...
        if (scramble_enabled_) {
          ...
          temp_ul = reinterpret_cast<int8_t*>(
              GetInfoBits(ul_bits_, Direction::kUplink, i, j, k));
        ...
        if (kDebugPrintBytes) {
          std::cout << "LDPC input print original; symbol index is: " << i
                    << " , UE id is: " << j << std::endl;
          ...
          std::cout << "LDPC input print after scramble; symbol index is: " << i
                    << " , UE id is: " << j << std::endl;
          for (size_t ii = 0; ii < ul_num_bytes_per_cb_; ii++) {
        ...
        if (i >= num_client_ul_pilot) {
          infile.read(reinterpret_cast<char*>(coded_bits_ptr),
                      ul_encoded_bytes_per_block);
          if (!infile) {
