...
void DataGenerator::DoDataGeneration(const std::string& directory) {
  ...
  ops_mp = rte_bbdev_op_pool_create("RTE_BBDEV_OP_LDPC_ENC_poo",
                                    RTE_BBDEV_OP_LDPC_ENC, NUM_ELEMENTS_IN_POOL,
  ...
  int rte_alloc_ref =
      rte_bbdev_enc_op_alloc_bulk(ops_mp, ref_enc_op, num_ul_codeblocks);
  ...
  if (num_ul_mac_bytes > 0) {
    ...
    std::vector<std::vector<int8_t>> ul_encoded_codewords_flexRAN(
        num_ul_codeblocks);
        ...
        if (kPrintEncodedBytes) {
          ...
          for (size_t n = 0; n < num_ul_codeblocks; n++) {
            std::printf("encoded bytes Symbol %zu, UE %zu\n",
                        n / this->cfg_->UeAntNum(), n % this->cfg_->UeAntNum());
            for (size_t i = 0; i < encoded_bytes; i++) {
              std::printf("%02X ", static_cast<uint8_t>(
                                       ul_encoded_codewords.at(n).at(i)));
            }
            std::printf("\n");
          }
          ...
          for (size_t n = 0; n < num_ul_codeblocks; n++) {
            ...
            for (size_t i = 0; i < encoded_bytes; i++) {
              std::printf("%02X ",
                          static_cast<uint8_t>(
                              ul_encoded_codewords_flexRAN.at(n).at(i)));
            }
            std::printf("\n");
          }
        }
