...
int memcmp_avx2S44VF(const uint8_t *data1, const uint8_t *data2,
                     size_t num_bytes) {
...
void print_uint32S44VF(const uint32_t *array, size_t totalByteLength) {
...
void print_casted_uint8_hexS44VF(const uint8_t *array, size_t totalByteLength) {
...
DoDecode_ACCS44VF::DoDecode_ACCS44VF(
...
DoDecode_ACCS44VF::~DoDecode_ACCS44VF() { std::free(resp_var_nodes_); }

int DoDecode_ACCS44VF::allocate_buffers_on_socket(
...
EventData DoDecode_ACCS44VF::Launch(size_t tag) {
  const LDPCconfig &ldpc_config = cfg_->LdpcConfig(Direction::kUplink);
  ...
  if (total == num_ul_syms * num_ue - 1) {
    ...
    if (enq_2 != 4 * num_ue) {
      ...
      enq_4 += rte_bbdev_enqueue_ldpc_dec_ops(dev_ids[3], 0,
                                              &ref_dec_op[3][enq_4], 1);
    }
    ...
    while (deq_4 < enq_4 && retry_count < MAX_DEQUEUE_TRIAL) {
      deq_4 += rte_bbdev_dequeue_ldpc_dec_ops(3, 0, &ops_deq[3][deq_4],
                                              enq_4 - deq_4);
      retry_count++;
    }
      ...
      for (size_t temp_idx = 0; temp_idx < 4 * num_ue; temp_idx++) {
        ...
        if (ref_symbol_idx < num_ul_syms) {
          ...
          if (print_frame_info) {
            ...
            print_casted_uint8_hexS44VF(temp_data, num_bytes_per_cb);
            std::cout << "\n\nContent of the ref_bytes are:\n";
            print_casted_uint8_hexS44VF(
                reinterpret_cast<const uint8_t *>(ref_byte), num_bytes_per_cb);
            std::cout << std::endl << std::endl;
          }
          size_t memcmp_ret = memcmp_avx2S44VF(
