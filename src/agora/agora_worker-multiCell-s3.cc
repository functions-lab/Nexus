...
#include "agora_worker-multiCell-s3.h"
...
AgoraWorkerS3::AgoraWorkerS3(Config* cfg, MacScheduler* mac_sched, Stats* stats,
...
AgoraWorkerS3::~AgoraWorkerS3() { JoinThreads(); }

void AgoraWorkerS3::CreateThreads() {
  ...
  for (size_t i = 0; i < config_->WorkerThreadNum(); i++) {
    workers_.emplace_back(&AgoraWorkerS3::WorkerThread, this, i);
  }
}

void AgoraWorkerS3::JoinThreads() {
...
void AgoraWorkerS3::WorkerThread(int tid) {
  ...
  auto compute_decoding =
      std::make_unique<DoDecode_ACCS3>(config_, tid, buffer_->GetDemod(),
