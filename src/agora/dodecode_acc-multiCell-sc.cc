...
DoDecode_ACC::DoDecode_ACC(
  ...
  std::string acc_addr;
  acc_addr = cfg_->ACC100Addr_1();
  std::cout << "acc address for this config is: " << acc_addr << std::endl;
  std::cout << "bbdev id is: " << (int)cfg_->bbdev_id_1() << std::endl;

  // const char *rte_argv[] = {"txrx",        "-l",           core_list.c_str(), "-a", acc_addr.c_str(),
  ...
  int rte_alloc_ref =
      rte_bbdev_dec_op_alloc_bulk(ops_mp, ref_dec_op, num_ul_syms * num_ue);
...
EventData DoDecode_ACC::Launch(size_t tag) {
  ...
  if ((symbol_idx_ul == num_ul_syms - 1) && (ue_id == num_ue - 1)) {
    ...
    for (size_t temp_ue_id = 0; temp_ue_id < num_ue; temp_ue_id++) {
      for (size_t temp_idx = 0; temp_idx < num_ul_syms; temp_idx++) {
        ...
        char *data_out =
            rte_pktmbuf_append(m_head_out, ldpc_config.NumCbCodewLen());
    ...
    for (enq = 0, deq = 0; enq < (num_ul_syms * num_ue);) {
      enq += rte_bbdev_enqueue_ldpc_dec_ops(dev_id, 0, &ref_dec_op[enq], 1);
      deq +=
          rte_bbdev_dequeue_ldpc_dec_ops(dev_id, 0, &ops_deq[deq], enq - deq);
    }
    ...
    if ((kEnableMac == false) && (kPrintPhyStats == true) &&
      ...
      for (size_t temp_idx = 2; temp_idx < num_ul_syms; temp_idx++) {
        for (size_t temp_ue_id = 0; temp_ue_id < num_ue; temp_ue_id++) {
          ...
          size_t memcmp_ret =
              memcmp(reinterpret_cast<uint8_t *>(temp_data),
  ...
  // std::cout << "enq numer is: " << enq << std::endl;
  if (enq == (num_ul_syms * num_ue - 1)) {
    size_t start_tsc = GetTime::WorkerRdtsc();
    ...
    AGORA_LOG_INFO("Cell # %d: \tFrame %d, ACC100: enq = %d, deq = %d\n",
                   (int)cfg_->bbdev_id_1(), frame_id, enq, deq);
    ...
    if ((kEnableMac == false) && (kPrintPhyStats == true) &&
      ...
      for (size_t temp_idx = 0; temp_idx < num_ul_syms; temp_idx++) {
        ...
        for (size_t temp_ue_id = 0; temp_ue_id < num_ue; temp_ue_id++) {
          ...
          if (ref_symbol_idx < num_ul_syms and ref_ue_idx < num_ue) {
            // std::cout <<"ref ue_idx is: " << ref_ue_idx << std::endl;
            ref_byte = cfg_->GetInfoBits(cfg_->UlBits(), Direction::kUplink,
            ...
            if (print_frame_info) {
              ...
              print_casted_uint8_hex(
                  reinterpret_cast<const uint8_t *>(ref_byte),
                  num_bytes_per_cb);
              std::cout << std::endl << std::endl;
            }
            size_t memcmp_ret = memcmp_avx2(
      ...
      AGORA_LOG_INFO("Cell # %d: \tframe %d, error is: %d\n",
                     (int)cfg_->bbdev_id_1(), frame_id, error - (num_ue * 2));
    }
    ...
    AGORA_LOG_INFO(
        "Cell # %d: \tframe %d, BLER memcmp check time is: %.2f ms\n",
        (int)cfg_->bbdev_id_1(), frame_id,
        GetTime::CyclesToUs(duration_3, cfg_->FreqGhz() * 1000));
      ...
      if (kPrintMbufData) {
        ...
        if (remaining_bytes > 0) {
          ...
          for (size_t j = 0; j < remaining_bytes; j++) {
            value |= static_cast<uint32_t>(mbuf_data[num_32bit_values * 4 + j])
                     << (j * 8);
          }
          std::printf("0x%08x", value);
        }
