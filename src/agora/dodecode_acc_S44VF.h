...
#ifndef DODECODE_ACCS44VF_H_
#define DODECODE_ACCS44VF_H_
...
class DoDecode_ACCS44VF : public Doer {
 public:
  DoDecode_ACCS44VF(
 ...
 private:
  ...
  uint16_t deq_2 = 0;
  size_t enq_index = 0;
  size_t enq_index_2 = 0;

  uint16_t enq_3 = 0;
  uint16_t deq_3 = 0;
  size_t enq_index_3 = 0;

  uint16_t enq_4 = 0;
  uint16_t deq_4 = 0;
  size_t enq_index_4 = 0;
  // const size_t num_ul_syms = cfg_->Frame().NumULSyms();

  struct rte_mempool* ops_mp[NUM_VF];
  struct rte_mempool* in_mbuf_pool[NUM_VF];
  struct rte_mempool* out_mbuf_pool[NUM_VF];
  struct rte_mempool* bbdev_op_pool[NUM_VF];

  struct rte_mbuf* in_mbuf;
  struct rte_mbuf* out_mbuf;

  // std::vector<std::vector<struct rte_bbdev_dec_op*>> ref_dec_ops;

  struct rte_bbdev_dec_op* ref_dec_op[NUM_VF][32];
  struct rte_bbdev_dec_op* ops_deq[NUM_VF][32];
