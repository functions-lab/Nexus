...
int memcmp_avx2S3(const uint8_t *data1, const uint8_t *data2,
                  size_t num_bytes) {
...
void print_uint32S3(const uint32_t *array, size_t totalByteLength) {
...
void print_casted_uint8_hexS3(const uint8_t *array, size_t totalByteLength) {
...
DoDecode_ACCS3::DoDecode_ACCS3(
  ...
  // AGORA_LOG_INFO("ACC100: Dev id is: %d, socket id is: %d\n", dev_id, (int)info.socket_id);
  ...
  ops_mp = rte_bbdev_op_pool_create(pool_name.c_str(), RTE_BBDEV_OP_LDPC_DEC,
                                    NUM_ELEMENTS_IN_POOL, OPS_CACHE_SIZE,
                                    (int)socket_id);
  if (ops_mp == nullptr) {
  ...
  in_mbuf_pool =
      rte_pktmbuf_pool_create(in_pool_name.c_str(), NUM_ELEMENTS_IN_MEMPOOL, 0,
                              0, DATA_ROOM_SIZE, (int)socket_id);
  out_mbuf_pool =
      rte_pktmbuf_pool_create(out_pool_name.c_str(), NUM_ELEMENTS_IN_MEMPOOL, 0,
                              0, DATA_ROOM_SIZE, (int)socket_id);
  ...
  std::cout << "int dev_id is: " << (int)dev_id << std::endl;
  int ret_socket_in = allocate_buffers_on_socket(
      inputs, 1 * sizeof(struct rte_bbdev_op_data), (int)socket_id);
  int ret_socket_hard_out = allocate_buffers_on_socket(
      hard_outputs, 1 * sizeof(struct rte_bbdev_op_data), (int)socket_id);
...
DoDecode_ACCS3::~DoDecode_ACCS3() { std::free(resp_var_nodes_); }

int DoDecode_ACCS3::allocate_buffers_on_socket(
...
EventData DoDecode_ACCS3::Launch(size_t tag) {
  const LDPCconfig &ldpc_config = cfg_->LdpcConfig(Direction::kUplink);
  ...
  enq += rte_bbdev_enqueue_ldpc_dec_ops((uint16_t)dev_id, (uint16_t)dev_id,
                                        &ref_dec_op[enq], 1);

  int retry_count = 0;
  while (deq < enq && retry_count < MAX_DEQUEUE_TRIAL) {
    deq += rte_bbdev_dequeue_ldpc_dec_ops(dev_id, dev_id, &ops_deq[deq],
                                          enq - deq);
    retry_count++;
  }
  // AGORA_LOG_INFO("ACC100: Dev id is: %d, enq = %d, deq = %d\n", dev_id, enq, deq);
  ...
  if ((kEnableMac == false) && (kPrintPhyStats == true) &&
    ...
    if (kPrintACC100Byte) {
      if (frame_id > 100 && frame_id % 100 == 0) {
        ...
        print_casted_uint8_hexS3(temp_data, num_bytes_per_cb);
    ...
    if (kPrintACC100Byte) {
      if (frame_id > 100 && frame_id % 100 == 0) {
        ...
        print_casted_uint8_hexS3(reinterpret_cast<uint8_t *>(ref_byte),
                                 num_bytes_per_cb);
        // print_uint32(temp_data, num_bytes_per_cb);
        std::cout << std::endl << std::endl;
      }
    ...
    // }
    size_t memcmp_ret =
        memcmp_avx2S3(reinterpret_cast<uint8_t *>(temp_data),
