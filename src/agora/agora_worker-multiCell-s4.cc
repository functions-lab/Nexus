...
#include "agora_worker-multiCell-s4.h"
...
AgoraWorkerS4::AgoraWorkerS4(Config* cfg, MacScheduler* mac_sched, Stats* stats,
...
AgoraWorkerS4::~AgoraWorkerS4() { JoinThreads(); }

void AgoraWorkerS4::CreateThreads() {
  ...
  for (size_t i = 0; i < config_->WorkerThreadNum(); i++) {
    workers_.emplace_back(&AgoraWorkerS4::WorkerThread, this, i);
  }
}

void AgoraWorkerS4::JoinThreads() {
...
void AgoraWorkerS4::WorkerThread(int tid) {
  ...
  if (tid == 0) {
    ...
    auto compute_decoding = std::make_unique<DoDecode_ACCS42VF>(
        config_, tid, buffer_->GetDemod(), buffer_->GetDecod(), phy_stats_,
        stats_);
    ...
    while (config_->Running() == true) {
      for (size_t i = 0; i < computers_vec.size(); i++) {
        if (computers_vec.at(i)->TryLaunchS2(
                *message_->GetTaskQueue_S2(events_vec.at(i), cur_qid),
                message_->GetCompQueue_S2(cur_qid),
                message_->GetWorkerPtok_S2(cur_qid, tid))) {
          empty_queue = false;
          break;
        }
      }
      ...
      if (empty_queue == true) {
        ...
        if (empty_queue_itrs == 5) {
          if (frame_->cur_sche_frame_id_S2_ != frame_->cur_proc_frame_id_S2_) {
            cur_qid ^= 0x1;
          } else {
            cur_qid = (frame_->cur_sche_frame_id_S2_ & 0x1);
          }
          empty_queue_itrs = 0;
        }
      } else {
