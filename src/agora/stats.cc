...
std::string Stats::PrintPerFrame(std::string const& doer_string,
  ...
  // std::cout <<"all threads are: " << frame_summary.count_all_threads_ << std::endl;
  ...
  if (kIsWorkerTimingEnabled) {
    ...
    if (kStatsPrintFrameSummary) {
      std::string print_summary =
          "Cell # " + std::to_string((int)config_->bbdev_id_1()) + ":\t" +
...
void Stats::SaveToFile() {
  AGORA_LOG_INFO("Cell # %d:\tStats: Saving master timestamps to %s\n",
                 (int)config_->bbdev_id_1(), kStatsDataFilename.c_str());
  ...
  if (kIsWorkerTimingEnabled == true) {
    AGORA_LOG_INFO("Cell # %d:\tStats: Printing detailed results to %s\n",
...
void Stats::PrintSummary() {
  AGORA_LOG_INFO("Cell # %d:\tStats: total processed frames %zu\n",
                 (int)config_->bbdev_id_1(), this->last_frame_id_ + 1);
  if (kIsWorkerTimingEnabled == false) {
    AGORA_LOG_INFO(
        "Cell # %d:\tStats: Worker timing is disabled. Not printing summary\n",
        (int)config_->bbdev_id_1());
  } else {
    ...
    if (config_->Frame().NumULSyms() > 0) {
      ...
      std::printf("Cell # %d:\tUplink totals (tasks, frames): ",
                  (int)config_->bbdev_id_1());
      std::printf("CSI (%zu, %.2f), ",
                  num_tasks.at(static_cast<size_t>(DoerType::kCSI)),
                  csi_frames);
  ...
  if (kDebugPrintPerFrameDone == true) {
    switch (print_type) {
      case (PrintType::kPacketRXPilots):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Received all pilots\n",
            (int)config_->bbdev_id_1(), frame_id,
            MasterGetDeltaMs(TsType::kPilotAllRX, TsType::kFirstSymbolRX,
                             frame_id));
        break;
      case (PrintType::kPacketRX):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Received all packets\n",
            (int)config_->bbdev_id_1(), frame_id,
      ...
      case (PrintType::kProcessingStart):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.4f ms]: Started processing\n",
            (int)config_->bbdev_id_1(), frame_id,
            MasterGetDeltaMs(TsType::kProcessingStarted, TsType::kFirstSymbolRX,
                             frame_id));
        break;
      case (PrintType::kFFTPilots):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.4f ms]: FFT-ed all pilots\n",
            (int)config_->bbdev_id_1(), frame_id,
            MasterGetDeltaMs(TsType::kFFTPilotsDone, TsType::kFirstSymbolRX,
                             frame_id));
        break;
      case (PrintType::kFFTCal):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: FFT-ed all calibration "
            "symbols\n",
            (int)config_->bbdev_id_1(), frame_id,
            MasterGetUsSince(TsType::kRCAllRX, frame_id) / 1000.0);
        break;
      case (PrintType::kBeam):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Completed %s beamweight "
            "calc\n",
            (int)config_->bbdev_id_1(), frame_id,
      ...
      case (PrintType::kDemul):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Completed demodulation\n",
            (int)config_->bbdev_id_1(), frame_id,
            MasterGetDeltaMs(TsType::kDemulDone, TsType::kFirstSymbolRX,
                             frame_id));
        break;
      case (PrintType::kDecode):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.4f ms]: Completed LDPC decoding "
            "(%zu UL "
            "symbols)\n",
            (int)config_->bbdev_id_1(), frame_id,
      ...
      case (PrintType::kPacketFromMac):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Completed MAC RX \n",
            (int)config_->bbdev_id_1(), frame_id,
            MasterGetMsSince(TsType::kFirstSymbolRX, frame_id));
        break;
      case (PrintType::kEncode):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Completed LDPC encoding\n",
            (int)config_->bbdev_id_1(), frame_id,
            MasterGetDeltaMs(TsType::kEncodeDone, TsType::kFirstSymbolRX,
                             frame_id));
        break;
      case (PrintType::kPrecode):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Completed precoding\n",
            (int)config_->bbdev_id_1(), frame_id,
            MasterGetDeltaMs(TsType::kPrecodeDone, TsType::kFirstSymbolRX,
                             frame_id));
        break;
      case (PrintType::kIFFT):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Completed IFFT\n",
            (int)config_->bbdev_id_1(), frame_id,
            MasterGetDeltaMs(TsType::kIFFTDone, TsType::kFirstSymbolRX,
                             frame_id));
        break;
      case (PrintType::kPacketTXFirst):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Completed TX of first "
            "symbol\n",
            (int)config_->bbdev_id_1(), frame_id,
      ...
      case (PrintType::kPacketTX):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Completed TX (%zu DL "
            "Control & %zu DL "
            "Data symbols)\n",
            (int)config_->bbdev_id_1(), frame_id,
      ...
      case (PrintType::kBroadcast):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Completed Generating (%zu "
            "DL Control "
            "symbols)\n",
            (int)config_->bbdev_id_1(), frame_id,
      ...
      case (PrintType::kPacketToMac):
        AGORA_LOG_INFO(
            "Cell # %d:\tMain [frame %zu + %.2f ms]: Completed MAC TX \n",
            (int)config_->bbdev_id_1(), frame_id,
            MasterGetMsSince(TsType::kFirstSymbolRX, frame_id));
        break;
      default:
