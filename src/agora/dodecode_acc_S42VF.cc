...
int memcmp_avx2S42VF(const uint8_t *data1, const uint8_t *data2,
                     size_t num_bytes) {
...
void print_uint32S42VF(const uint32_t *array, size_t totalByteLength) {
...
void print_casted_uint8_hexS42VF(const uint8_t *array, size_t totalByteLength) {
...
DoDecode_ACCS42VF::DoDecode_ACCS42VF(
  ...
  std::string core_list =
      std::to_string(LCORE_ID);  // this is hard set to core 36
  ...
  if (ret < 0) {
    printf("rte_bbdev_setup_queues(%u, %u, %d) ret %i\n", dev_ids[0],
           NUM_QUEUES, rte_socket_id(), ret);
  }

  for (int i = 0; i < NUM_VF; i++) {
    ret = rte_bbdev_setup_queues(i, NUM_QUEUES, i);
  }
  ...
  for (int i = 0; i < NUM_VF; i++) {
    ...
    ops_mp[i] =
        rte_bbdev_op_pool_create(pool_name.c_str(), RTE_BBDEV_OP_LDPC_DEC,
                                 NUM_ELEMENTS_IN_POOL, OPS_CACHE_SIZE, i);
  }

  if (ops_mp[0] == nullptr or ops_mp[1] == nullptr) {
  ...
  for (int i = 0; i < NUM_VF; i++) {
    ...
    in_mbuf_pool[i] =
        rte_pktmbuf_pool_create(pool_name_in.c_str(), NUM_ELEMENTS_IN_MEMPOOL,
                                0, 0, DATA_ROOM_SIZE, (int)socket_ids[i]);
    out_mbuf_pool[i] =
        rte_pktmbuf_pool_create(pool_name_out.c_str(), NUM_ELEMENTS_IN_MEMPOOL,
                                0, 0, DATA_ROOM_SIZE, (int)socket_ids[i]);
  }

  if (in_mbuf_pool[0] == nullptr or out_mbuf_pool[0] == nullptr or
      in_mbuf_pool[1] == nullptr or out_mbuf_pool[1] == nullptr) {
    std::cerr << "Error: Unable to create mbuf pool: "
              << rte_strerror(rte_errno) << std::endl;
  }

  int rte_alloc_ref = rte_bbdev_dec_op_alloc_bulk(ops_mp[0], ref_dec_op[0],
                                                  num_ul_syms * num_ue);
  if (rte_alloc_ref != TEST_SUCCESS) {
    rte_exit(EXIT_FAILURE, "Failed to alloc bulk\n");
  }

  rte_alloc_ref = rte_bbdev_dec_op_alloc_bulk(ops_mp[1], ref_dec_op[1],
                                              num_ul_syms * num_ue);
  if (rte_alloc_ref != TEST_SUCCESS) {
    rte_exit(EXIT_FAILURE, "Failed to alloc bulk\n");
  }
  ...
  const struct rte_bbdev_op_cap *cap = info[0].drv.capabilities;
  const struct rte_bbdev_op_cap *capabilities = NULL;
  rte_bbdev_info_get(dev_ids[0], &info[0]);
  ...
  const struct rte_bbdev_op_cap *cap_2 = info[1].drv.capabilities;
  const struct rte_bbdev_op_cap *capabilities_2 = NULL;
  rte_bbdev_info_get(dev_ids[1], &info[1]);
  for (unsigned int i = 0; cap_2->type != RTE_BBDEV_OP_NONE; ++i, ++cap_2) {
    std::cout << "cap is: " << cap_2->type << std::endl;
    if (cap_2->type == RTE_BBDEV_OP_LDPC_DEC) {
      capabilities_2 = cap_2;
      std::cout << "capability is being set to: " << capabilities_2->type
                << std::endl;
      break;
    }
  }
  ...
  for (int i = 0; i < iter_num; i++) {
    for (int j = 0; j < 2; j++) {
      ...
      if (!check_bit(ref_dec_op[j][i]->ldpc_dec.op_flags,
        ...
        ref_dec_op[j][i]->ldpc_dec.op_flags +=
            RTE_BBDEV_LDPC_ITERATION_STOP_ENABLE;
      }
      ref_dec_op[j][i]->ldpc_dec.iter_max =
          (uint8_t)ldpc_config.MaxDecoderIter();
...
DoDecode_ACCS42VF::~DoDecode_ACCS42VF() { std::free(resp_var_nodes_); }

int DoDecode_ACCS42VF::allocate_buffers_on_socket(
    struct rte_bbdev_op_data **buffers, const int len, const int socket) {
  int i;
...
EventData DoDecode_ACCS42VF::Launch(size_t tag) {
  const LDPCconfig &ldpc_config = cfg_->LdpcConfig(Direction::kUplink);
  ...
  if ((enq == (num_ul_syms * num_ue) / 2 &&
       enq_2 == (num_ul_syms * num_ue) / 2 - 1) or
      (enq_2 == (num_ul_syms * num_ue) / 2 &&
       enq == (num_ul_syms * num_ue) / 2 - 1)) {
    ...
    if (enq == (num_ul_syms * num_ue) / 2) {
      ...
      enq_2 += rte_bbdev_enqueue_ldpc_dec_ops(dev_ids[1], 0,
                                              &ref_dec_op[1][enq_2], 1);
    } else {
    ...
    while (deq_2 < enq_2 && retry_count < MAX_DEQUEUE_TRIAL) {
      deq_2 += rte_bbdev_dequeue_ldpc_dec_ops(1, 0, &ops_deq[1][deq_2],
                                              enq_2 - deq_2);
      retry_count++;
    }
    ...
    if ((kEnableMac == false) && (kPrintPhyStats == true) &&
      ...
      for (size_t temp_idx = 0; temp_idx < num_ul_syms / 2; temp_idx++) {
        bool print_frame_info =
            kPrintACC100Byte && frame_id > 100 && frame_id % 100 == 0;
        ...
        for (size_t temp_ue_id = 0; temp_ue_id < num_ue; temp_ue_id++) {
          ...
          if (ref_symbol_idx < num_ul_syms) {
            ref_byte = cfg_->GetInfoBits(cfg_->UlBits(), Direction::kUplink,
                                         ref_symbol_idx, ref_ue_idx, cur_cb_id);
            if (print_frame_info) {
              std::cout << "CB size = " << num_bytes_per_cb << " bytes\n"
                        << "Content of the CB (in uint32):\n";
              print_casted_uint8_hexS42VF(temp_data, num_bytes_per_cb);
              std::cout << "\n\nContent of the ref_bytes are:\n";
              print_casted_uint8_hexS42VF(
                  reinterpret_cast<const uint8_t *>(ref_byte),
                  num_bytes_per_cb);
              std::cout << std::endl << std::endl;
            }
            size_t memcmp_ret = memcmp_avx2S42VF(
                reinterpret_cast<uint8_t *>(temp_data),
                reinterpret_cast<uint8_t *>(ref_byte), num_bytes_per_cb);
            if (memcmp_ret != 0) {
      ...
      for (size_t temp_ue_id = 0; temp_ue_id < num_ue; temp_ue_id++) {
        phy_stats_->UpdateDecodedBits(temp_ue_id, symbol_offset, frame_slot,
                                      num_bytes_per_cb * 8);
        phy_stats_->IncrementDecodedBlocks(temp_ue_id, symbol_offset,
                                           frame_slot);
        phy_stats_->UpdateBlockErrors(temp_ue_id, symbol_offset, frame_slot,
    ...
    if (symbol_idx_ul % 2 == 0) {
      ...
      if (symbol_idx_ul == 14 && frame_id > 100 && frame_id % 100 == 0 &&
          kPrintLLRData) {
        ...
        if (kPrintMbufData) {
          ...
          if (remaining_bytes > 0) {
            ...
            for (size_t j = 0; j < remaining_bytes; j++) {
              value |=
                  static_cast<uint32_t>(mbuf_data[num_32bit_values * 4 + j])
                  << (j * 8);
            }
            std::printf("0x%08x", value);
          }
        ...
        if (memcmp(mbuf_data, llr_buffer_ptr, data_length) == 0) {
          ...
          for (size_t j = 0; j < data_length; ++j) {
            if (mbuf_data[j] != (uint8_t)llr_buffer_ptr[j]) {
              std::printf(
                  "Mismatch at byte %zu: mbuf_data=%02x, llr_buffer_ptr=%02x\n",
