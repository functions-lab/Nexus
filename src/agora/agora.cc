...
{
  if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
    agora_memory_ = std::make_unique<AgoraBuffer>(cfg);
  } else {
    agora_memory_ = std::make_unique<AgoraBufferS2>(cfg);
  }
}
...
void Agora::Stop() {
  AGORA_LOG_INFO("Cell # %d:\tAgora: terminating\n",
                 (int)config_->bbdev_id_1());
...
void Agora::ScheduleAntennas(EventType event_type, size_t frame_id,
  ...
  for (size_t i = 0; i < num_blocks; i++) {
    ...
    if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
      ...
      message_S2_->EnqueueEventTaskQueue_S2(event_type, qid, event);
    }
  }
}
  ...
  for (const auto& worker : worker_events) {
    if (!worker.empty()) {
      ...
      if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
        ...
        TryEnqueueBulkFallback(message_S2_->GetTxConQ_S2(),
                               message_S2_->GetTxPTokPtr_S2(enqueue_worker_id),
                               worker.data(), worker.size());
      }
    }
    enqueue_worker_id++;
...
void Agora::ScheduleSubcarriers(EventType event_type, size_t frame_id,
  ...
  for (size_t i = 0; i < num_events; i++) {
    if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
      ...
      message_S2_->EnqueueEventTaskQueue_S2(
          event_type, qid, EventData(event_type, base_tag.tag_));
    }
...
void Agora::ScheduleBroadCastSymbols(EventType event_type, size_t frame_id) {
  ...
  if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
    ...
    message_S2_->EnqueueEventTaskQueue_S2(event_type, qid,
                                          EventData(event_type, base_tag.tag_));
  }
}
...
void Agora::TryScheduleFft() {
  if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
    ...
    // std::cout << "before % kFrameWnd, result is: " << frame_tracking_.cur_sche_frame_id_ << std::endl;
    // std::cout <<"in sc: " << frame_tracking_.cur_sche_frame_id_ % kFrameWnd << std::endl;
    ...
    if (cur_fftq.size() >= config_->FftBlockSize()) {
      ...
      for (size_t i = 0; i < num_fft_blocks; i++) {
        ...
        for (size_t j = 0; j < config_->FftBlockSize(); j++) {
          ...
          // std::cout <<"frame_tracking_.cur_sche_frame_id_ = " << frame_tracking_.cur_sche_frame_id_ << std::endl;
    ...
    const auto& cfg = this->config_;
    // std::cout << "before % kFrameWnd, result is: " << frame_tracking_S2_.cur_sche_frame_id_S2_ << std::endl;
    // std::cout <<"in s2: " << frame_tracking_S2_.cur_sche_frame_id_S2_ % kFrameWnd << std::endl;
    std::queue<fft_req_tag_t>& cur_fftq =
        fft_queue_arr_.at(frame_tracking_S2_.cur_sche_frame_id_S2_ % kFrameWnd);
    const size_t qid = frame_tracking_S2_.cur_sche_frame_id_S2_ & 0x1;

    // std::cout <<"cur fft queue is: " << cur_fftq.size()
    if (cur_fftq.size() >= config_->FftBlockSize()) {
      const size_t num_fft_blocks = cur_fftq.size() / config_->FftBlockSize();
      for (size_t i = 0; i < num_fft_blocks; i++) {
        EventData do_fft_task;
        ...
        for (size_t j = 0; j < config_->FftBlockSize(); j++) {
          ...
          // std::cout <<"frame_tracking_S2_.cur_sche_frame_id_ = " << frame_tracking_S2_.cur_sche_frame_id_S2_ << std::endl;
          if (this->fft_created_count_ == 0) {
            this->stats_->MasterSetTsc(
                TsType::kProcessingStarted,
                frame_tracking_S2_.cur_sche_frame_id_S2_);
            stats_->PrintPerFrameDone(PrintType::kProcessingStart,
                                      frame_tracking_S2_.cur_sche_frame_id_S2_);
          }
          this->fft_created_count_++;
          if (this->fft_created_count_ == rx_counters_.num_rx_pkts_per_frame_) {
            ...
            if (cfg->BigstationMode() == true) {
              this->CheckIncrementScheduleFrame(
                  frame_tracking_S2_.cur_sche_frame_id_S2_, kUplinkComplete);
            }
          }
        }
        message_S2_->EnqueueEventTaskQueue_S2(EventType::kFFT, qid,
                                              do_fft_task);
      }
    }
  }
  ...
  for (size_t i = 0; i < config_->SocketThreadNum(); i++) {
    if (remaining_events > 0) {
      ...
      if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
        ...
        const size_t new_events =
            message_S2_->GetRxConQ_S2()->try_dequeue_bulk_from_producer(
                *(message_S2_->GetRxPTokPtr_S2(i)),
                &events_list.at(total_events), request_events);
        remaining_events = remaining_events - new_events;
        total_events = total_events + new_events;
      }
    } else {
      if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
        ...
        AGORA_LOG_WARN(
            "remaining_events = %zu:%zu, queue %zu num elements %zu\n",
            remaining_events, total_events, i,
            message_S2_->GetRxConQ_S2()->size_approx());
      }
    }
  }
...
size_t Agora::FetchDoerEvent(std::vector<EventData>& events_list) {
  if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
    ...
    return message_S2_->DequeueEventCompQueueBulk_S2(
        frame_tracking_S2_.cur_proc_frame_id_S2_ & 0x1, events_list);
  }
}
...
void Agora::Start() {
  ...
  if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
    ...
    frame_tracking_S2_.cur_sche_frame_id_S2_ = 0;
    frame_tracking_S2_.cur_proc_frame_id_S2_ = 0;
  }
  ...
  if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
    ...
    message_S2_ = std::make_unique<MessageInfoS2>(
        kDefaultWorkerQueueSize * config_->Frame().NumDataSyms(),
        kDefaultMessageQueueSize * config_->Frame().NumDataSyms(),
        config_->SocketThreadNum());
  }
  ...
  // finish:
  AGORA_LOG_INFO("Cell # %d:\tAgora: printing stats and saving to file\n",
                 (int)config_->bbdev_id_1());
...
void Agora::HandleEvents(EventData& event, size_t& tx_count, double tx_begin,
  ...
  switch (event.event_type_) {
    case EventType::kPacketRX: {
      ...
      size_t temp;
      if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
        temp = frame_tracking_.cur_sche_frame_id_ + kFrameWnd;
      } else {
        temp = frame_tracking_S2_.cur_sche_frame_id_S2_ + kFrameWnd;
      }
      if (pkt->frame_id_ >= temp) {
        if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
          AGORA_LOG_ERROR(
          ...
          AGORA_LOG_ERROR(
              ...
              "frame window (= %zu + %zu). This can happen if "
              "Agora is running slowly, e.g., in debug mode\n",
              pkt->frame_id_, frame_tracking_S2_.cur_sche_frame_id_S2_,
              kFrameWnd);
        }
        cfg->Running(false);
        break;
        ...
        if (last_demul_symbol == true) {
          ...
          if (kUplinkHardDemod) {
            if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
              ...
              assert(frame_tracking_S2_.cur_proc_frame_id_S2_ == frame_id);
            }
            ...
            if (cfg->BigstationMode() == false) {
              ...
              // std::cout <<"before kuplink complete------------" << std::endl;
              CheckIncrementScheduleFrame(frame_id, kUplinkComplete);
            } else {
      ...
      if (last_decode_task == true) {
        ...
        if (last_decode_symbol == true) {
          ...
          if (kEnableMac == false) {
            ...
            const bool work_finished = this->CheckFrameComplete(frame_id);
            // std::cout << "work finised is : " << work_finished << std::endl;
            if (work_finished == true) {
              // goto finish;
              finish = true;
              return;
    ...
    case EventType::kPacketFromMac: {
      ...
      if (last_ue == true) {
        ...
        size_t temp;
        if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
          temp = frame_tracking_.cur_proc_frame_id_ + kScheduleQueues;
        } else {
          temp = frame_tracking_S2_.cur_proc_frame_id_S2_ + kScheduleQueues;
        }
        if ((this->encode_deferral_.empty() == false) || (frame_id >= temp)) {
          if (kDebugDeferral) {
        ...
        if (last_ifft_task == true) {
          ...
          if (last_ifft_symbol == true) {
            ...
            if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
              ...
              assert(frame_id == frame_tracking_S2_.cur_proc_frame_id_S2_);
            }
        ...
        if (tx_count == tx_counters_.MaxSymbolCount() * 9000) {
          ...
          if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
            ...
            AGORA_LOG_INFO(
                ...
                samples_num_per_ue, cfg->SpatialStreamsNum(), diff,
                samples_num_per_ue * std::log2(16.0f) / diff,
                message_S2_->GetTxConQ_S2()->size_approx());
          }
          unused(diff);
...
void Agora::UpdateRxCounters(size_t frame_id, size_t symbol_id) {
  ...
  if (rx_counters_.num_pkts_.at(frame_slot) == 0) {
    if (kEnableMac == false) {
      ...
      if ((encode_deferral_.empty() == false) || (frame_id >= temp)) {
        if (kDebugDeferral) {
          AGORA_LOG_INFO("   +++ Deferring encoding of frame %zu\n", frame_id);
        }
    ...
    if (kDebugPrintPerFrameStart) {
      ...
      AGORA_LOG_INFO(
          "Cell # %d:\tMain [frame %zu + %.2f ms since last frame]: Received "
          "first packet. Remaining packets in prev frame: %zu\n",
          (int)config_->bbdev_id_1(), frame_id,
...
void Agora::InitializeThreads() {
  ...
  if (kUseArgos || kUseUHD || kUsePureUHD) {
    if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
      ...
      packet_tx_rx_ = std::make_unique<PacketTxRxRadio>(
          config_, config_->CoreOffset() + 1, message_S2_->GetRxConQ_S2(),
          message_S2_->GetTxConQ_S2(), message_S2_->GetRxPTokPtr_S2(),
          message_S2_->GetTxPTokPtr_S2(), agora_memory_->GetUlSocket(),
          agora_memory_->GetUlSocketSize() / config_->PacketLength(),
          this->stats_->FrameStart(), agora_memory_->GetDlSocket());
    }
#if defined(USE_DPDK)
  ...
  std::cout << "solution in config file is: " << config_->Solution()
            << std::endl;

  if (config_->Solution() == "sc") {
    AGORA_LOG_INFO("Creating SC worker\n");
    worker_ = std::make_unique<AgoraWorkerSC>(
        config_, mac_sched_.get(), stats_.get(), phy_stats_.get(),
        message_.get(), agora_memory_.get(), &frame_tracking_);
  } else if (config_->Solution() == "s2") {
    AGORA_LOG_INFO("Creating S2 worker\n");
    worker_ = std::make_unique<AgoraWorkerS2>(
        config_, mac_sched_.get(), stats_.get(), phy_stats_.get(),
        message_S2_.get(), agora_memory_.get(), &frame_tracking_S2_);
  } else if (config_->Solution() == "s1") {
    AGORA_LOG_INFO("Creating S1 worker\n");
    worker_ = std::make_unique<AgoraWorkerS1>(
        config_, mac_sched_.get(), stats_.get(), phy_stats_.get(),
        message_S2_.get(), agora_memory_.get(), &frame_tracking_S2_);
  } 
  else if (config_->Solution() == "sw") {
    AGORA_LOG_INFO("Creating Pure SW worker\n");
    worker_ = std::make_unique<AgoraWorkerSW>(
        config_, mac_sched_.get(), stats_.get(), phy_stats_.get(),
        message_S2_.get(), agora_memory_.get(), &frame_tracking_S2_);} 
  else if (config_->Solution() == "scsw") {
    AGORA_LOG_INFO("Creating SCSW worker\n");
    worker_ = std::make_unique<AgoraWorkerSCSW>(
        config_, mac_sched_.get(), stats_.get(), phy_stats_.get(),
        message_.get(), agora_memory_.get(), &frame_tracking_);
  } else if (config_->Solution() == "s3") {
    AGORA_LOG_INFO("Creating S3 worker\n");
    worker_ = std::make_unique<AgoraWorkerS3>(
        config_, mac_sched_.get(), stats_.get(), phy_stats_.get(),
        message_S2_.get(), agora_memory_.get(), &frame_tracking_S2_);
  } else if (config_->Solution() == "s4") {
    AGORA_LOG_INFO("Creating S4 worker\n");
    worker_ = std::make_unique<AgoraWorkerS4>(
        config_, mac_sched_.get(), stats_.get(), phy_stats_.get(),
        message_S2_.get(), agora_memory_.get(), &frame_tracking_S2_);
  } else {
    std::cout << "sorry, currently solution not supported" << std::endl;
    std::exit(0);
  }
...
void Agora::CheckIncrementScheduleFrame(size_t frame_id,
  ...
  //     static_cast<uint8_t>(ScheduleProcessingFlags::kUplinkComplete)) {
  if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
    this->frame_tracking_.cur_sche_frame_id_++;
  } else {
    this->frame_tracking_S2_.cur_sche_frame_id_S2_++;
  }
  ...
  if (this->config_->Frame().NumDLSyms() == 0) {
    this->schedule_process_flags_ += ScheduleProcessingFlags::kDownlinkComplete;
  }
}
// }
...
bool Agora::CheckFrameComplete(size_t frame_id) {
  ...
  if ((true == this->ifft_counters_.IsLastSymbol(frame_id)) &&
    ...
    // std::cout << "before cur proc adding---------------------" << std::endl;
    if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
      frame_tracking_.cur_proc_frame_id_++;
    } else {
      // std::cout <<"in s2, before adding cur_proc_frame_id" << std::endl;
      frame_tracking_S2_.cur_proc_frame_id_S2_++;
    }
      ...
      for (size_t encode = 0;
        ...
        if (deferred_frame < temp) {
          ...
          if (config_->Solution() == "sc" or config_->Solution() == "scsw") {
            ...
            RtAssert(deferred_frame >= frame_tracking_S2_.cur_proc_frame_id_S2_,
                     "Error scheduling encoding because deferral frame is less "
                     "than current frame");
          }
