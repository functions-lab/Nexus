...
int memcmp_avx2S43VF(const uint8_t *data1, const uint8_t *data2,
                     size_t num_bytes) {
...
void print_uint32S43VF(const uint32_t *array, size_t totalByteLength) {
...
void print_casted_uint8_hexS43VF(const uint8_t *array, size_t totalByteLength) {
...
DoDecode_ACCS43VF::DoDecode_ACCS43VF(
  ...
  for (int i = 0; i < NUM_VF; i++) {
    ...
    ops_mp[i] = rte_bbdev_op_pool_create(
        pool_name.c_str(), RTE_BBDEV_OP_LDPC_DEC, NUM_ELEMENTS_IN_POOL,
        OPS_CACHE_SIZE, (int)socket_ids[i]);
    if (ops_mp[i] == nullptr) {
  ...
  int rte_alloc_ref;
  for (int i = 0; i < NUM_VF; i++) {
    rte_alloc_ref = rte_bbdev_dec_op_alloc_bulk(ops_mp[i], ref_dec_op[i],
                                                num_ul_syms * num_ue);
...
DoDecode_ACCS43VF::~DoDecode_ACCS43VF() { std::free(resp_var_nodes_); }

int DoDecode_ACCS43VF::allocate_buffers_on_socket(
...
EventData DoDecode_ACCS43VF::Launch(size_t tag) {
  const LDPCconfig &ldpc_config = cfg_->LdpcConfig(Direction::kUplink);
  ...
  if (total == num_ul_syms * num_ue - 1) {
    ...
    if (enq == 6 * num_ue and enq_2 == 5 * num_ue) {
    ...
    } else if (enq_2 == 5 * num_ue and enq_3 == 5 * num_ue) {
      ...
      enq +=
          rte_bbdev_enqueue_ldpc_dec_ops(dev_ids[0], 0, &ref_dec_op[0][enq], 1);
    } else {
      m_head = rte_pktmbuf_alloc(in_mbuf_pool[0]);
      ...
      enq_3 += rte_bbdev_enqueue_ldpc_dec_ops(dev_ids[2], 0,
                                              &ref_dec_op[2][enq_3], 1);
    }
    ...
    while (deq_3 < enq_3 && retry_count < MAX_DEQUEUE_TRIAL) {
      deq_3 += rte_bbdev_dequeue_ldpc_dec_ops(2, 0, &ops_deq[2][deq_3],
                                              enq_3 - deq_3);
      retry_count++;
    }
    ...
    if ((kEnableMac == false) && (kPrintPhyStats == true) &&
      ...
      for (size_t temp_idx = 0; temp_idx < 6 * num_ue; temp_idx++) {
        ...
        if (ref_symbol_idx < num_ul_syms) {
          ...
          if (print_frame_info) {
            ...
            print_casted_uint8_hexS43VF(temp_data, num_bytes_per_cb);
            std::cout << "\n\nContent of the ref_bytes are:\n";
            print_casted_uint8_hexS43VF(
                reinterpret_cast<const uint8_t *>(ref_byte), num_bytes_per_cb);
            std::cout << std::endl << std::endl;
          }
          size_t memcmp_ret = memcmp_avx2S43VF(
      ...
      for (size_t temp_idx = 0; temp_idx < 5 * num_ue; temp_idx++) {
        bool print_frame_info =
            kPrintACC100Byte && frame_id > 100 && frame_id % 100 == 0;
    ...
    AGORA_LOG_INFO("Error VF1 = %d, VF2 = %d, VF3 = %d\n", error_1 - num_ue,
                   error_2 - num_ue, error_3);
    error_1 = 0;
    error_2 = 0;
    error_3 = 0;
  } else {
    if (symbol_idx_ul % 3 == 0) {
