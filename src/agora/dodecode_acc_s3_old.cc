...
DoDecode_ACC::DoDecode_ACC(
  ...
  std::cout << "socket id is: " << static_cast<int>(info.socket_id)
            << std::endl;
...
EventData DoDecode_ACC::Launch(size_t tag) {
  ...
  if (kDebugPrintInTask == true) {
    std::printf(
        "In doDecode thread %d: frame: %zu,, : %zu, code block: "
        "%zu, ue: %zu offset %zu\n",
        tid_, frame_id, symbol_id, cur_cb_id, ue_id, symbol_offset);
  }

  std::cout << "in_tid is: " << in_tid_ << std::endl;

  if (enq == ((num_ul_syms * num_ue) / 2 - 1)) {
    std::cout << "tid is : " << in_tid_
              << ", symbol idx ul is: " << symbol_idx_ul << std::endl;
